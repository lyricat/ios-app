...
extension PinSettingsViewController {
    ...
    private func title(for interval: TimeInterval) -> String {
        ...
        if interval < hour {
            return R.string.localizable.wallet_pin_pay_interval_minutes_count(Int(interval / 60))
        } else if interval == hour {
            return R.string.localizable.one_hour()
        } else {
            return R.string.localizable.wallet_pin_pay_interval_hours_count(Int(interval / 3600))
        }
    }
